...
Skroll.prototype.inViewport = function(elem,settings){
	var win = $(this.settings.reference);
...
Skroll.prototype.init = function(){
	var _this = this;
	if(!this.settings.mobile && $(window).width() < 600) return _this;
	...
	$(_this.settings.reference).on("resize scroll",_this.throttle(function(){
		$.each(_this.elements,function(key,val){
	...
	$(this.settings.reference).scroll();
	$(this.settings.reference).resize(this.recalcPosition());
	return _this;
}
